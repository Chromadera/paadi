...
export const ChangePINInputScreen: React.FC = () => {
  ...
  const handlePress = (digit: string) => {
    if (pin.length < 4) {
      ...
      setPin(newPin);
      if (newPin.length === 4) setTimeout(() => navigation.navigate('ConfirmNewPIN'), 200);
    }
  };
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top', 'bottom']}>
      {/* Header — matches HTML: back ← (20px), title "Change PIN" (17px bold), 20px spacer */}
      ...
      <View style={styles.form}>
        {/* Current PIN row — pre-filled dots (from previous screen) */}
        ...
        {/* New PIN row — interactive dots for new PIN entry */}
