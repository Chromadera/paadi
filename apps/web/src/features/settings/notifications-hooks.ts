...
import { NotificationPreferenceInput } from "@paadi/contracts";
...
export function useUpdateNotificationPrefs() {
  ...
  return useMutation({
    mutationFn: (updatedList: NotificationPreferenceInput[]) =>
      getAuthedClient().updateNotificationPreferences({ preferences: updatedList }),
    
    // Optional Optimistic Update to make the toggle feel instant and high-performance
    onMutate: async (updatedList) => {
      await queryClient.cancelQueries({ queryKey: QUERY_KEY });
      const previousPrefs = queryClient.getQueryData<{ preferences: NotificationPreferenceInput[] }>(QUERY_KEY);
